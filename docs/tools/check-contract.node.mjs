#!/usr/bin/env node
/**
 * ⚠️ NODE-ONLY GOVERNANCE TOOL
 * This file runs in CI (external Node) or local audits. It must not be executed
 * inside the GAIS browser runtime. Node APIs and filesystem access are used here
 * intentionally for audit tooling only.
 * Location: /docs/tools - this is the only approved Node-area in the repository.
 */
...
for (const file of specFiles) {
  ...
  const partnerMatch = txt.match(/TDD Partner:\s*`([^`]+)`/i) || txt.match(/\(See\s+`([^`]+)`\)/i) || txt.match(/See\s+`(tdd-[^`]+\.md)`/i);
